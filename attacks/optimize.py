...
class Optimization():
    ...
    def optimize(self, w_batch, targets_batch, num_epochs):
        ...
        for i in range(num_epochs):
            ...
            mean = torch.mean(torch.abs(w_batch.grad)).item()
            std = torch.std(torch.abs(w_batch.grad)).item()
            print("Epoch {}: {:.4f}e-6 +- {:.4f}e-6".format(i, mean*1e6, std*1e6))
            analysis_gradient["mean"].append(mean)
            analysis_gradient["std"].append(std)
        ...
        return w_batch.detach(), analysis_gradient
