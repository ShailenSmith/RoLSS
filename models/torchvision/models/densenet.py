...
class _DenseLayer(nn.Module):
    ...
    def bn_function(self, inputs: List[Tensor]) -> Tensor:
        ...
        # print(f"Concat shape:{concated_features.shape}")
    ...
    def forward(self, input: Tensor) -> Tensor:  # noqa: F811
        ...
        if self.memory_efficient and self.any_requires_grad(prev_features):
            ...
            # print(f"Length of prev_features: {len(prev_features)}")
            # print(f"Shape of prev_features: {prev_features[-1].shape}")
            bottleneck_output = self.bn_function(prev_features)
            # print(f"Shape of bottleneck_output: {bottleneck_output.shape}")
...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            skipped_num_input_features = round(self.skip * num_input_features)
            skipped_growth_rate = round(self.skip * growth_rate)
            ...
            if i == 0:
                ...
                layer = _DenseLayer(
                    skipped_num_input_features + (i-1) * skipped_growth_rate + growth_rate,
                    growth_rate=growth_rate,
                    bn_size=bn_size,
                    drop_rate=drop_rate, 
                    memory_efficient=memory_efficient,
    ...
    def forward(self, init_features: Tensor) -> Tensor:
        ...
        for name, layer in self.items():
            ...
            channel_idx = round(channels * self.skip)
            features[-1] = features[-1][:,:channel_idx,:,:]
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        memory_efficient: bool = False,
        # skip: List[int] = [1,1,1,1],
        skip: Optional[List[float]] = [1,1,1,1],
    ) -> None:
        ...
        for i, num_layers in enumerate(block_config):
            ...
            # num_features_in = num_features + num_layers * growth_rate
            # num_features_out = num_features_in // 2

            ## SCALING SKIP IMPLEMENTATION ##
            skipped_num_features = round(skip[i] * num_features)
            skipped_growth_rate = round(skip[i] * growth_rate)

            num_features_in = skipped_num_features + (num_layers-1) * skipped_growth_rate + growth_rate
            num_features_out = num_features_in // 2
            ...
            #     num_features_out = num_features_in // 2
            # else:
            #     num_features_in = growth_rate
            #     num_features_out = num_features_in // 2
            
            if i != len(block_config) - 1:
                # trans = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
                trans = _Transition(num_input_features=num_features_in, num_output_features=num_features_out)
