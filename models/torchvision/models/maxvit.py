...
from models.torchvision.models._api import register_model, Weights, WeightsEnum
from models.torchvision.models._meta import _IMAGENET_CATEGORIES
from models.torchvision.models._utils import _ovewrite_named_param, handle_legacy_interface
from models.torchvision.ops.misc import Conv2dNormActivation, SqueezeExcitation
from models.torchvision.ops.stochastic_depth import StochasticDepth
from models.torchvision.transforms._presets import ImageClassification, InterpolationMode
from models.torchvision.utils import _log_api_usage_once
...
class PartitionAttentionLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        ### REMOVE SKIP CONNECTION ###
        if self.mode != "mlp" and not self.cfg:
            x = self.stochastic_dropout(self.attn_layer(x)) + self.skip * x
        else:
            x = self.stochastic_dropout(self.attn_layer(x)) + x

        if self.mode != "attention" and not self.cfg:
            x = self.stochastic_dropout(self.mlp_layer(x)) + self.skip * x
        else:
            x = self.stochastic_dropout(self.mlp_layer(x)) + x
        ...
        #     x = self.stochastic_dropout(self.mlp_layer(x))
...
class MaxVitLayer(nn.Module):
    ...
    def __init__(
        ...
        mbconv_cfg, block_cfg, grid_cfg = layer_cfg
...
class MaxVit(nn.Module):
    ...
    def __init__(
        ...
        # One-hot list for skip connection in maxvit blocks
        skip: Optional[List[int]] = [1,1,1,1],
        # Mode of skip connection removal
        mode: Optional[str] = None,
        cfgs: Optional[List[List[int]]] = None,
        ...
        # Ensure that user input for mode is valid
        valid_modes = ["all", "attention", "mlp"]
        if mode:
        ...
        if cfgs is None:
            cfgs = [[1,1,1] for i in range(4)]
        
        if itrs is None:
            itrs = block_layers

        for in_channel, out_channel, num_layers, skip_bool, cfg, itr in zip(in_channels, out_channels, block_layers, skip, cfgs, itrs):
            self.blocks.append(
                MaxVitBlock(
